...
class LSTMModel(BaseModel, nn.Module):
    ...
    def prepare_dataset(self, encoded_seqs):
        ...
        for seq in encoded_seqs:
            inputs.append(torch.tensor(seq[:-1], dtype=torch.long).to(self.device))  # Move data to the device
            targets.append(seq[-1])

        inputs_padded = pad_sequence(inputs, batch_first=True, padding_value=0)
        targets = torch.tensor(targets, dtype=torch.long).to(self.device)  # Move data to the device
